Public Class cKPI_QUINCENAS
    Inherits cBase
    Implements IDisposable, ITemporal

#Region "Constantes Privadas"
    Private Const strConsulta = "SELECT QUINCENA, NOMBRE " & _
                                "FROM KPI_QUINCENAS " & _
                                "WHERE 1=1 "
#End Region
    ...
    Public Function ObtenerValores(ByVal pnDesde As Integer, ByVal pnHasta As Integer) As DataTable Implements ITemporal.ObtenerValores
        ...
        Try
            ...
            With SqlConsulta
                .Connection = dbconexion
                .CommandType = CommandType.Text
                .CommandText = "SELECT Z.EJERCICIO, Z.MES, Z.NOMBRE_MES, Z.QUINCENA, Z.NOMBRE_QUINCENA, Z.VALOR FROM ( " & _
                               "SELECT ROW_NUMBER() OVER() ORDEN, X.EJERCICIO, X.MES, X.NOMBRE_MES, X.QUINCENA, X.NOMBRE_QUINCENA, X.VALOR FROM ( " & _
                               "SELECT A.EJERCICIO, A.MES, A.NOMBRE_MES, A.QUINCENA, A.NOMBRE_QUINCENA, B.VALOR " & _
                               "FROM (SELECT A.EJERCICIO, B.MES, C.QUINCENA, B.NOMBRE NOMBRE_MES, C.NOMBRE NOMBRE_QUINCENA FROM KPI_EJERCICIOS A, KPI_MESES B, KPI_QUINCENAS C WHERE TO_DATE (B.MES || '/' || A.EJERCICIO, 'MM/YYYY') <= CURRENT_DATE ORDER BY A.EJERCICIO DESC, B.MES DESC, C.QUINCENA DESC ) A " & _
                               "LEFT JOIN KPI_DATASET_VALUES B ON A.EJERCICIO = B.EJERCICIO AND A.MES = B.MES AND A.QUINCENA = B.QUINCENA " & _
                               "AND B.DATASETID = :P_DATASETID " & _
                               "ORDER BY A.EJERCICIO DESC, A.MES DESC, A.QUINCENA DESC ) X ) Z WHERE Z.ORDEN BETWEEN :P_DESDE AND :P_HASTA "
