Public Class cKPI_TRIMESTRES
    Inherits cBase
    Implements IDisposable, ITemporal

#Region "Constantes Privadas"
    Private Const strConsulta = "SELECT TRIMESTRE, SEMESTRE, NOMBRE	" & _
                                "FROM KPI_TRIMESTRES " & _
                                "WHERE 1=1 "
#End Region
    ...
    Public Function ObtenerValores(ByVal pnDesde As Integer, ByVal pnHasta As Integer) As DataTable Implements ITemporal.ObtenerValores
        ...
        Try
            ...
            With SqlConsulta
                .Connection = dbconexion
                .CommandType = CommandType.Text
                .CommandText = "SELECT Z.EJERCICIO, Z.TRIMESTRE, Z.NOMBRE_TRIMESTRE, Z.VALOR FROM ( " & _
                               "SELECT ROW_NUMBER() OVER() ORDEN, X.EJERCICIO, X.TRIMESTRE, X.NOMBRE_TRIMESTRE, X.VALOR FROM ( " & _
                               "SELECT A.EJERCICIO, A.TRIMESTRE, A.NOMBRE NOMBRE_TRIMESTRE, B.VALOR " & _
                               "FROM (SELECT DISTINCT A.EJERCICIO, B.TRIMESTRE, B.NOMBRE FROM KPI_EJERCICIOS A, KPI_TRIMESTRES B, KPI_MESES C WHERE B.TRIMESTRE = C.TRIMESTRE AND TO_DATE (C.MES || '/' || A.EJERCICIO, 'MM/YYYY') <= CURRENT_DATE) A " & _
                               "LEFT JOIN KPI_DATASET_VALUES B ON A.EJERCICIO = B.EJERCICIO AND A.TRIMESTRE = B.TRIMESTRE " & _
                               "AND B.DATASETID = :P_DATASETID " & _
                               "ORDER BY A.EJERCICIO DESC, A.TRIMESTRE DESC ) X ) Z WHERE Z.ORDEN BETWEEN :P_DESDE AND :P_HASTA "
