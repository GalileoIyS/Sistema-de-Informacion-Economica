    ...
    Public Function ObtenerValores(ByVal pnDesde As Integer, ByVal pnHasta As Integer) As DataTable Implements ITemporal.ObtenerValores
        ...
        Try
            ...
            With SqlConsulta
                .Connection = dbconexion
                .CommandType = CommandType.Text
                .CommandText = "SELECT X.EJERCICIO, X.MES, X.NOMBRE_MES, X.DIA, X.VALOR FROM ( " & _
                               "SELECT ROW_NUMBER() OVER() AS ORDEN, Z.EJERCICIO, Z.MES, Z.NOMBRE_MES, Z.DIA, Z.VALOR FROM ( " & _
                               "SELECT A.EJERCICIO, A.MES, A.NOMBRE_MES, A.DIA, B.VALOR " & _
                               "FROM (SELECT X.EJERCICIO, Y.MES, Z.DIA, Y.NOMBRE NOMBRE_MES FROM KPI_EJERCICIOS X, KPI_MESES Y, KPI_DIAS Z WHERE TO_DATE (Y.MES || '/' || X.EJERCICIO, 'MM/YYYY') <= CURRENT_DATE) A " & _
                               "LEFT JOIN KPI_DATASET_VALUES B ON A.EJERCICIO = B.EJERCICIO AND A.MES = B.MES AND A.DIA = B.DIA " & _
                               "AND B.DATASETID = :P_DATASETID " & _
                               "ORDER BY A.EJERCICIO DESC, A.MES DESC, A.DIA DESC ) Z ) X WHERE X.ORDEN BETWEEN :P_DESDE AND :P_HASTA "
