...
<script runat="server">
	...
	public override bool CheckAuthentication()
	{
		...
		//
        if (Session["ContentFolder"] == null)
            return false;
        
        if ((Context.User.IsInRole("administrador")) || (Context.User.IsInRole("editor")) || (Context.User.IsInRole("kpi")))
            return true;
		//
	...
	public override void SetConfig()
	{
		...
		// Ruta http que se va a utilizar para buscar la imagen en el servidor.
        BaseUrl = HttpContext.Current.Request.Url.GetLeftPart(UriPartial.Authority) + HttpRuntime.AppDomainAppVirtualPath + "/uploads/" + Session["ContentFolder"];

		// Ruta física en el disco donde debe almacenar la imagen que acaba de subir al servidor.
		BaseDir = HttpContext.Current.Request.PhysicalApplicationPath.Replace("\\","/") + "/uploads/" + Session["ContentFolder"];
